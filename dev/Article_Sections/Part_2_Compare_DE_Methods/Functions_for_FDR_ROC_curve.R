...
#(Section 6)
#Create function which
#Returns FDR /ROC statistics from log likelihoods from bayseq 
...
Bayseq_analysis<-function(simulated_data) {
  ...
  replicates <-sapply(seq(from = -5, to=5, by = 0.5), 
...
#Function which takes Bayseq log_likelihoods and then uses this to produce FDR/ ROC table
...
FDR_ROC_Bayseq<-function(table, simulated_data) {
  ...
  # FDR_pred<-sapply(seq(from=0, to=1, by=1/1000), function(x) {
  #   table %>% 
  #   as_tibble() %>% 
  #   mutate(V1 = 1-V1) %>% 
  #   dplyr::rename(PEP = 1-V1) %>% 
  #   arrange(PEP) %>% 
  ...
  #Calculate Number of Genes declared non-significant at each probability value
  
  N_non_sig<-sapply(N_sig, function(x) {sim_genes_size - x} )
  ...
  #TP<-sapply(FP, function(x) {N_sig - x})
  ...
  #                 filter(V1<(1-x))
  #               
  #               Non_sig<-full_table %>% filter(grepl("V",names)) %>% nrow()
  #               
  #               #Total<-full_table %>% nrow()
  #               
  #               return(Non_sig)
