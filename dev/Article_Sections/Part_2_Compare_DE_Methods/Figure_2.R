...
#Use log likelihoods to obtain FDR curve values  

FDR_Bayseq_df<-lapply(Posterior_Bayseq, function(x) {
  
  FDR_Bayseq(x %>% 
               exp(), Fig_2_sim_data_sample_105 %>% 
               dplyr::rename(Sample = sample))
...
saveRDS(FDR_ROC_Bayseq_df, compress = "gzip", file = "FDR_ROC_Bayseq_df.rds")
...
TABI_2000_iter_Fig_2_sim_data <- readRDS("~/TABI/dev/Article_Figures/Figure_2/TABI_2000_iter_Fig_2_sim_data.rds")
