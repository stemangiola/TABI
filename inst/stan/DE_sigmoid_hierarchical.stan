...
data {
	...
	real<lower=0> multiplier[T,G];           // Scale factor 
...
transformed data{
	...
	real < lower =0 > caux = 1;
		//Log of TMM factor
	real log_multiplier[T,G];
	
		//log_TMM
	for(t in 1:T) for(g in 1:G) log_multiplier[t,g] = log(multiplier[t,g]);
}
...
model {
	...
	// Likelihood - fiting 
	if(prior_only == 0) for(t in 1:T) y[t] ~ neg_binomial_2_log(to_vector(log_multiplier[t])+log_y_hat[t], 1 ./ exp(phi[t]));
...
generated quantities{
  int y_gen[T,G];          // RNA-seq counts
	...
	for (t in 1:T) {
	  for(g in 1:G) {
	  y_gen[t,g] = neg_binomial_2_log_rng(log_multiplier[t,g]+log_y_hat[t,g], 1 ./ exp(phi[t,g])); 
	  } 
	  }
