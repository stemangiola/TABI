functions{
  ...
  vector log_gen_inv_logit(row_vector y_log, row_vector b0, vector y_cross, vector A) {
     vector[rows(y_log)] x = log(-A+y_cross) + log1p_exp(-to_vector(b0)) - log1p_exp(- to_vector(y_log)  ) ;
...
parameters {
	...
	vector[G] y_cross; //y_cross (No Bounds)
...
transformed parameters {
  ...
  beta[2] = to_row_vector(beta1_z[1]);
	...
	for(t in 1:T) y_hat[t] = log_gen_inv_logit(X[t] * beta, beta[1], y_cross, A);
...
model {
	...
	y_cross ~ normal(0,5); //gamma_log(exp(log_y_cross_prior[1]) * inv(exp(log_y_cross_prior[2])), inv(exp(log_y_cross_prior[2])) );
