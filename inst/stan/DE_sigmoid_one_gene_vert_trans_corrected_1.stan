functions{
  ...
  vector gen_inv_logit(row_vector y_log, row_vector b0, vector log_y_cross, vector A) {
     vector[rows(y_log)] x = log(-A+exp(log_y_cross)) + log1p_exp(-to_vector(b0)) - log1p_exp(- to_vector(y_log)  ) ;
...
transformed parameters {
  ...
  beta[2] = to_row_vector(beta1_z[1]);
	...
	for(t in 1:T) y_hat[t] = gen_inv_logit(X[t] * beta, beta[1], log_y_cross, A);
...
model {
	...
	log_y_cross ~ normal(0,1); //gamma_log(exp(log_y_cross_prior[1]) * inv(exp(log_y_cross_prior[2])), inv(exp(log_y_cross_prior[2])) );
